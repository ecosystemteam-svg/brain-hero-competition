  ...
  return (
    <div className="floating-score" style={{ '--start-x': `${randomX}%`, '--start-y': `${randomY}%`, '--duration': `${randomDuration}s`, '--delay': `${randomDelay}s`, background: tier.bgGradient }}>
      <div className="score-bubble">
        <div className="score-top"><span className="score-emoji">{tier.emoji}</span><span className="chrono-emoji">{chronotype?.emoji || '🧠'}</span></div>
...
const InputFormView = ({ onSubmit, onCancel, isSubmitting }) => {
  ...
  return (
    <div className="form-view">
      <div className="form-container">
        <div className="form-header"><div className="brain-icon animate-bounce">🧠</div><h2>ลงทะเบียน Brain Hero</h2><p>กรอกผล CogMate ของคุณ</p></div>
        <div className="form-content">
          <div className="form-group"><label>ชื่อผู้เข้าแข่งขัน</label><input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="ใส่ชื่อของคุณ" className="input-field" /></div>
            ...
            <div className="chronotype-grid">
              {Object.entries(chronotypeConfig).map(([key, value]) => (
                <button key={key} type="button" className={`chronotype-btn ${formData.chronotype === key ? 'active' : ''}`} onClick={() => setFormData({...formData, chronotype: key})}>
                  <span className="chrono-emoji">{value.emoji}</span><span className="chrono-name">{value.name}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="form-group age-section">
            ...
            <div className="age-inputs">
              <div className="age-input-group"><span className="score-type">อายุจริง</span><input type="text" inputMode="numeric" value={formData.realAge} onChange={(e) => setFormData({...formData, realAge: e.target.value.replace(/[^0-9]/g, '')})} onFocus={(e) => e.target.select()} className="input-field" placeholder="ปี" /></div>
              <div className="age-input-group"><span className="score-type">อายุสมอง</span><input type="text" inputMode="numeric" value={formData.brainAge} onChange={(e) => setFormData({...formData, brainAge: e.target.value.replace(/[^0-9]/g, '')})} onFocus={(e) => e.target.select()} className="input-field" placeholder="ปี" /></div>
            </div>
            {hasAges && (
              <div className={`brain-gap-display ${brainGap >= 0 ? 'positive' : 'negative'}`}>
                ...
                <span className="gap-label">Brain Gap:</span><span className="gap-value">{brainGap >= 0 ? '+' : ''}{brainGap} ปี</span>
          ...
          {hasScores && (
            <div className="preview-card" style={{ background: tierConfig[preview.tier].bgGradient }}>
              <div className="preview-header"><span className="preview-emoji">{tierConfig[preview.tier].emoji}</span><span className="preview-tier">{tierConfig[preview.tier].name}</span><span className="preview-chrono">{chronotypeConfig[formData.chronotype].emoji}</span></div>
          ...
          <div className="form-actions">
            ...
            <button type="button" className="btn-primary" disabled={!formData.name.trim() || isSubmitting} onClick={handleSubmit}>{isSubmitting ? '⏳ กำลังส่ง...' : 'ส่งคะแนน 🚀'}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="tier-filter">
      {tiers.map(tier => (
        <button key={tier.key} className={`filter-btn ${selectedTier === tier.key ? 'active' : ''}`} onClick={() => onSelectTier(tier.key)} style={{ '--tier-color': tier.color }}>
          <span className="filter-emoji">{tier.emoji}</span><span className="filter-name">{tier.name}</span><span className="filter-count">{counts[tier.key] || 0}</span>
        </button>
      ))}
    </div>
  );
        ...
        {sorted.map((p, i) => {
          ...
          return (
            <div key={p.id} className={`leaderboard-item rank-${i + 1}`}>
              ...
              <div className="participant-info"><span className="participant-name">{p.name} {chronotypeConfig[p.chronotype]?.emoji}</span><span className="participant-tier" style={{ color: tier.color }}>{tier.emoji} {tier.name}</span></div>
...
const ScoreboardView = ({ participants, onAddDemo, onClearAll, onRefresh, filterTier, setFilterTier, musicPlaying, setMusicPlaying, isLoading }) => {
    ...
    <div className="scoreboard-view">
      ...
      <div className="scoreboard-main">
        <div className="floating-area" key={`float-${filterTier}`}>
          {isLoading && <div className="waiting-message"><span className="waiting-emoji">⏳</span><span>กำลังโหลด...</span></div>}
          {!isLoading && filteredForFloat.map((p, i) => <FloatingScore key={`${p.id}-${filterTier}`} participant={p} index={i} />)}
          {!isLoading && filteredForFloat.length === 0 && (
      ...
      <div className="control-buttons">
        ...
        <button className="refresh-btn" onClick={onRefresh}>🔄 รีเฟรช</button>
        <button className="demo-btn" onClick={onAddDemo}>➕ Demo</button>
        <button className="clear-btn" onClick={onClearAll}>🗑️ ล้าง</button>
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  useEffect(() => { fetchParticipants(); const interval = setInterval(fetchParticipants, 10000); return () => clearInterval(interval); }, []);

  const addParticipant = async (data) => {
    setIsSubmitting(true);
    try {
      await fetch(`${API_URL}?action=add&data=${encodeURIComponent(JSON.stringify(data))}`);
  ...
  const addDemoParticipant = async () => {
    const names = ['มานะ', 'มานี', 'ปิติ', 'ชูใจ', 'วิริยะ', 'สมชาย', 'สมหญิง', 'วิชัย', 'นภา', 'ธนา'];
    ...
    const data = { id: Date.now(), name: names[Math.floor(Math.random() * names.length)] + Math.floor(Math.random() * 100), attentionScore: profile.att, memoryScore: profile.mem, realAge: profile.rAge, brainAge: profile.bAge, chronotype: chronotypes[Math.floor(Math.random() * chronotypes.length)], timestamp: Date.now() };
    try { await fetch(`${API_URL}?action=add&data=${encodeURIComponent(JSON.stringify(data))}`); await fetchParticipants(); } catch (error) { console.error('Demo error:', error); }
  };

  const clearAll = async () => {
    if (confirm('ล้างข้อมูลทั้งหมด?')) {
      try { await fetch(`${API_URL}?action=clear`); setParticipants([]); } catch (error) { console.error('Clear error:', error); }
    }
  };

  if (isLoading && participants.length === 0) return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#1a1a2e', color: 'white', fontFamily: 'Prompt, sans-serif' }}><div style={{ textAlign: 'center' }}><span style={{ fontSize: '80px', display: 'block', animation: 'pulse 1s infinite' }}>🧠</span><p>กำลังโหลดข้อมูล...</p></div></div>;
    ...
    <div className="app-container">
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .demo-btn, .clear-btn, .music-btn, .refresh-btn { padding: 14px 20px; border: none; border-radius: 25px; color: white; font-family: inherit; font-size: 0.9rem; font-weight: 600; cursor: pointer; transition: all 0.3s; box-shadow: 0 4px 15px rgba(0,0,0,0.3); min-width: 120px; text-align: center; }
        ...
        .refresh-btn { background: linear-gradient(135deg, #f093fb, #f5576c); }
        .demo-btn:hover, .clear-btn:hover, .music-btn:hover, .refresh-btn:hover { transform: translateY(-3px); box-shadow: 0 6px 20px rgba(0,0,0,0.4); }
      ...
      {view === 'form' && <InputFormView onSubmit={addParticipant} onCancel={() => setView('scoreboard')} isSubmitting={isSubmitting} />}
      {view === 'scoreboard' && <ScoreboardView participants={participants} onAddDemo={addDemoParticipant} onClearAll={clearAll} onRefresh={fetchParticipants} filterTier={filterTier} setFilterTier={setFilterTier} musicPlaying={musicPlaying} setMusicPlaying={setMusicPlaying} isLoading={isLoading} />}
    </div>
  );
}
